...
**理想的な分離状態**（完全直交）では、以下の性質を持つ：

1. **対角成分が大きい**（$\theta_{pp} \approx 90°$）:
   同順位の主成分同士が離れている → メジャー成分が互いに分離されている

2. **非対角成分が小さい**（$\theta_{pq} \approx 0°$ for $p \neq q$）:
   異順位の主成分同士は近い → 重要度の異なる成分間では干渉しない

これは、**加重コーダル距離が「重要な成分のみを分離する」という思想と対応**する。

**不完全な分離状態**（0.9999問題）では、対角成分が90°に達せず、非対角成分も変動する可能性がある。
...
**理想的な分離**（λブースト、指標=1.0）:
- 対角成分（同順位ペア）: 赤色（$\approx 90°$）
- 非対角成分（異順位ペア）: 青色（$\approx 0°$）

**不完全な分離**（Dual Optimizer、指標=0.9999）:
- 対角成分: やや黄色寄り（$< 90°$）
- 非対角成分: 微妙に色がつく
